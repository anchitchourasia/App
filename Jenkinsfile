pipeline {
    ...
    environment {
        ...
        PROXY_CREDS = credentials('proxy-creds')
    ...
    stages {
        ...
        stage('Build Flutter App') {
            environment {
                HTTP_PROXY  = "http://${PROXY_CREDS_USR}:${PROXY_CREDS_PSW}@192.168.9.112:8080"
                HTTPS_PROXY = "http://${PROXY_CREDS_USR}:${PROXY_CREDS_PSW}@192.168.9.112:8080"
                http_proxy  = "http://${PROXY_CREDS_USR}:${PROXY_CREDS_PSW}@192.168.9.112:8080"
                https_proxy = "http://${PROXY_CREDS_USR}:${PROXY_CREDS_PSW}@192.168.9.112:8080"
            }
            steps {
